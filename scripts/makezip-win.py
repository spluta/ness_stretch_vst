...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\NessStretch Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\NessStretch Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\manual\\NessStretch manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\\NessStretch.vst3\\Contents\\x86_64-win\\NessStretch.vst3",
      projectpath + "\\build-win\\NessStretch_x64.exe"  
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\NessStretch-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\NessStretch-app_x64.pdb"  
  ]
